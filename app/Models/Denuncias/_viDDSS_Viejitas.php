...
class _viDDSS_Viejitas extends Model
{
...
//    use SoftDeletes;
    ...
    protected $table = '_viddss_viejitas';
...
//
    ...
    public function semaforo_ultimo_estatus()
    {
        ...
        if ($dias <= $this->dias_ejecucion) {
            $sem = 1;
            $class_color = 'text-verde-semaforo';
        }
        if ($dias > $this->dias_ejecucion && $dias <= $this->dias_maximos_ejecucion) {
            $sem = 2;
            $class_color = 'text-amarillo-semaforo';
        }
        if ($dias > $this->dias_maximos_ejecucion) {
            $sem = 3;
