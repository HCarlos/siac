...
class SabanaDeDatosUnityController extends Controller{
...
//            dd($Items);
            ...
            $file_external = "sabana_de_datos_unidad.xlsx";
            $arrFE = explode('.',$file_external);
            $newFileName = "sabana_de_datos_".$abreviatura."_" . Carbon::parse($start_date)->format('dmY') .'_'.Carbon::parse($end_date)->format('dmY'). '.xlsx';
...
//            dd($this->unidades);
            ...
            for($i=0, $iMax = count($this->unidades); $i< $iMax; $i++){
                ...
                $sh->setCellValue('E2', $this->unidad);
            ...
            for($i=0, $iMax = count($this->unidades); $i< $iMax; $i++){
                ...
                foreach ($filters as $item){
                    $sh
                        ...
                        ->setCellValue('G' . $C, $item["ultimo_estatus_compacto"][0]['TotalCantidad'] ?? 0)
                        ->setCellValue('H' . $C, $item["ultimo_estatus_compacto"][0]['Medida'] ?? 0);
    ...
    public function Sabana($C, $C0, $sh, $Items){
        ...
        $sh->setCellValue('C1', "RANGO DE CONSULTA:");
        $sh->setCellValue('D1', "DEL ".$this->start_date." AL ".$this->end_date);
        $sh->setCellValue('A2', "SOLICITUDES DE SERVICIO DE ".$this->unidad);
...
//                ->setCellValue('C' . $C, trim($Ciudadano ?? ''))
...
//                ->setCellValue('B' . $C, trim($item->referencia ?? ''))
//
...
//                ->setCellValue('AC' . $C, $Zona ?? '');
            ...
            if ($compactKey !== null) {
                ...
                $arrItemsBySue[$sueId]["ultimo_estatus_compacto"][$compactKey]["TotalCantidad"] += $item->cantidad;
                $arrItemsBySue[$sueId]["ultimo_estatus_compacto"][$compactKey]["Medida"] = $item->medida;
