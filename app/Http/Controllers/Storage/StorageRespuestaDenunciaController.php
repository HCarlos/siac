...
class StorageRespuestaDenunciaController extends Controller{
    ...
    public function subirArchivoDenunciaRespuesta(Request $request, $DenunciaObject, $RespuestaObject)
    {
...
//                    if ( $DenunciaObject->id == 0) {
                        ...
                        $this->attaches($item,$DenunciaObject,$RespuestaObject);
...
//                    }
    ...
    public function attaches($Item,$DenunciaObject,$RespuestaObject){
        $Item->users()->attach($Item->user__id);
        $den = Denuncia::find($DenunciaObject->id);
        $den->imagenes()->attach($Item);
        $RespuestaObject->imagenes()->attach($Item);
        return $Item;
    }

    public function detaches($Item,$DenunciaObject,$RespuestaObject){
        $Item->users()->detach($Item->user__id);
        $den = Denuncia::find($DenunciaObject->id);
        $den->imagenes()->detach($DenunciaObject->id);
        $RespuestaObject->imagenes()->detach($DenunciaObject->id);
