...
{{--        {{ Form::date('fecha', Carbon\Carbon::parse($item->fecha)->format('Y-m-d'), ['id'=>'fecha','class'=>'form-control']) }}--}}
        <input type="date" name="fecha" id="fecha" value="{{ \Carbon\Carbon::parse($items->fecha)->format('d-m-Y') }}" class="form-control"  >
...
@else
    <input type="hidden" id="user__id" name="user__id" value="{{ $item->user__id }}"/>
@endif
